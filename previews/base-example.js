...
class BaseExample {
    ...
    createLights(movePointLight = true) {
        ...
        let sphereLightMesh;
        if (movePointLight) {
            const sphereLightGeometry = new SphereGeometry(0.95);
        ...
        this.pointLight = new PointLight(0xffffff, 1.75);
        ...
        this.pointLight.tick = ms => {
            ...
            // Simulate seasonal changes by modifying the y-axis position over a longer cycle

            this.pointLight.position.x = Math.sin(angle) * radius;
            this.pointLight.position.z = Math.cos(angle) * radius;
            this.pointLight.position.y = height; // Add seasonal offset to height
